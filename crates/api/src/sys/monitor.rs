...
use crate::sys::{
    allocator::{memory_stats, memory_pressure, MemoryPressure, MemoryStats},
    interrupts::{TIME_MS, RANDTHING1, RANDTHING2},
    task::executor::{TASK_METRICS, TaskMetrics},
};
...
impl PerformanceMonitor {
    ...
    pub fn check_system_health(&mut self) -> HealthStatus {
        ...
        if current_time - self.last_memory_check > 5000 { // Check every 5 seconds
            ...
            if self.baseline_memory == 0 {
                ...
                if growth_rate > 100.0 { // More than 100KB/sec growth
                    self.record_event(PerformanceEvent::MemoryLeak { growth_rate_kb_per_sec: growth_rate });
                }
        ...
        let recent_critical_events = self.events.iter()
            .filter(|e| matches!(e, PerformanceEvent::MemoryPressureHigh | PerformanceEvent::TaskQueueFull))
            .count();
...
pub fn log_system_status() {
    ...
    log::info!("Tasks: {} spawned, {} completed", 
...
impl Profiler {
    ...
    pub fn finish(self) -> u64 {
        ...
        if duration > 10 { // Log functions taking more than 10ms
