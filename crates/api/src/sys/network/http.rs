...
impl HttpServer {
    ...
    pub fn handle_request(&self, request: &HttpRequest) -> HttpResponse {
        if let Some(handler) = self.routes.get(&request.path) {
            ...
            HttpResponse::new(HttpStatus::NotFound).with_body("<h1>404 Not Found</h1>".to_string())
        }
    }
}
...
pub fn setup_demo_server() -> AgaveResult<()> {
    ...
    server.route("/", |_req| {
        HttpResponse::new(HttpStatus::Ok).with_body(
            "<h1>Welcome to Agave OS!</h1><p>This is a demo HTTP server.</p>".to_string(),
        )
    });
