...
impl ThermalManager {
    ...
    pub fn update_thermal_state(
        &mut self,
        freq_scaler: &mut CpuFrequencyScaler,
    ) -> AgaveResult<()> {
    ...
    fn apply_cooling_policy(
        &mut self,
        policy: &CoolingPolicy,
        freq_scaler: &mut CpuFrequencyScaler,
    ) -> AgaveResult<()> {
        log::warn!(
            "Thermal policy '{}' activated - CPU temp: {:.1}°C",
            policy.name,
    ...
    pub fn get_temperature(&self, zone: &str) -> Option<f32> {
        self.thermal_zones
            .get(zone)
            .map(|zone| zone.temperature_celsius)
    }
...
impl PowerManager {
    ...
    pub fn update(&mut self) -> AgaveResult<()> {
        ...
        self.thermal_manager
            .update_thermal_state(&mut self.freq_scaler)?;

        // Scale CPU frequency based on policy and thermal state
        if !self.thermal_manager.is_thermal_throttling() {
            self.freq_scaler
                .scale_frequency(metrics.cpu_utilization_percent, &self.current_policy)?;
        }
    ...
    fn check_power_state_transitions(
        &mut self,
        now: u64,
        metrics: &crate::sys::monitor::SystemMetrics,
    ) -> AgaveResult<()> {
        ...
        let new_state = match (&self.current_policy, &self.current_state) {
            ...
            (_, PowerState::Sleep) => {
                if metrics.cpu_utilization_percent > 10.0 || time_since_activity < 1000 {
                    // Recent activity
                    PowerState::Active
                } else if time_since_activity > 30000 {
                    // 30 seconds in sleep
            ...
            (_, PowerState::DeepSleep) => {
                if metrics.cpu_utilization_percent > 5.0 || time_since_activity < 1000 {
                    PowerState::Active
                } else {
                    PowerState::DeepSleep
                }
        ...
        *self
        ...
        match &new_state {
            PowerState::Active => {
                ...
                self.freq_scaler
                    .set_frequency(self.freq_scaler.max_freq_mhz)?;
    ...
    fn log_power_status(&self) {
        log::info!(
            "Power Status - State: {:?}, CPU: {} MHz, Temp: {:.1}°C, Power: {:.1}W",
            self.current_state,
            self.freq_scaler.current_freq_mhz,
