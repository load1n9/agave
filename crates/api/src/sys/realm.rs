...
pub fn realm_exists(realm: &str) -> bool {
    ...
    crate::fs::exists(format!("/realms/{}", realm).as_str())
}
...
pub fn create_realm(realm: &str) {
    if !(realm_exists(realm) || realm == "bin") {
        ...
        crate::fs::create_dir(pathname.as_str());
    }
}
...
pub fn delete_realm(realm: &str) -> bool {
    if !realm_exists(realm) {
        ...
        crate::fs::delete(format!("/realms/{}", realm).as_str()).is_ok()
    }
}
...
pub fn get_realms() -> Vec<String> {
    ...
    for entry in crate::fs::read_dir("/realms").unwrap() {
        let entry = entry;
        let path = entry.name();
