// ported from https://github.com/theseus-os/Theseus/blob/theseus_main/kernel/memory/src/lib.rs
...
pub use crate::sys::page_alloc::{
    allocate_pages, allocate_pages_at, allocate_pages_by_bytes, allocate_pages_by_bytes_at,
    AllocatedPages,
};
use crate::sys::{frame_alloc, page_alloc};

pub use crate::sys::frame_alloc::{
...
pub use super::memory_x86_64::{
    find_section_memory_bounds, get_p4, tlb_flush_all, tlb_flush_virt_addr,
};
...
pub use crate::api::config::memory::PAGE_SIZE;
use crate::sys::boot_info::{BootInformation, MemoryRegion};
...
pub static KERNEL_MMI: Once<MmiRef> = Once::new();
...
pub const DMA_FLAGS: PteFlags = PteFlags::from_bits_truncate(
    PteFlags::new().bits() | PteFlags::VALID.bits() | PteFlags::WRITABLE.bits(),
);
...
pub fn map_frame_range<F: Into<PteFlagsArch>>(
    ...
    let kernel_mmi_ref =
        get_kernel_mmi_ref().ok_or("map_range(): KERNEL_MMI was not yet initialized!")?;
    let allocated_pages = allocate_pages_by_bytes(size_in_bytes)
        .ok_or("memory::map_range(): couldn't allocate contiguous pages!")?;
...
pub struct EarlyIdentityMappedPages {
    pub _init: MappedPages,
    pub _text: MappedPages,
    pub _rodata: MappedPages,
    pub _data: MappedPages,
}
...
pub fn init(
    ...
    #[cfg(target_arch = "x86_64")]
    {
        ...
        let (vga_start_paddr, vga_size, _vga_flags) = super::memory_x86_64::get_vga_mem_addr()?;
    ...
    let into_alloc_frames_fn = frame_alloc::init(
        free_regions.iter().flatten(),
        reserved_regions.iter().flatten(),
    )?;
    println!("Initialized new frame allocator!");
    frame_alloc::dump_frame_allocator_state();
    ...
    println!("Initialized new page allocator!");
    page_alloc::dump_page_allocator_state();

    // Initialize paging, which creates a new page table and maps all of the current code/data sections into it.
    super::paging::init(boot_info, kernel_stack_start, into_alloc_frames_fn)
}
...
pub fn init_post_heap(
    ...
    page_alloc::convert_to_heap_allocated();
    frame_alloc::convert_to_heap_allocated();
