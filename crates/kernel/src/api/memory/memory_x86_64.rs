// ported from https://github.com/theseus-os/Theseus/blob/theseus_main/kernel/memory_x86_64/src/lib.rs

use crate::sys::{
    boot_info::{BootInformation, ElfSection},
    pte_flags::PteFlags,
};

use super::structs::{PhysicalAddress, VirtualAddress};
use crate::api::config::memory::KERNEL_OFFSET;
    ...
    F: Fn(VirtualAddress) -> Option<PhysicalAddress>,
{
    ...
    for section in boot_info.elf_sections()? {
        ...
        if section.len() == 0
            || !section
                .flags()
                .contains(crate::sys::boot_info::ElfSectionFlags::ALLOCATED)
        ...
        let static_str_name = match section.name() {
            ...
            ".tbss" => {
                ...
                println!("     no need to map kernel section \".tbss\", it contains no content");
                continue;
            }
            ".data" => {
            ...
            ".page_table" | ".stack" => {
                println!("     no need to map this section, it is mapped separately later");
                continue;
            }
            ...
            ".bootloader-config" => {
                ...
                println!("     no need to map this section, it is only used by the bootloader for config.");
                continue;
            }
            _ => {
                error!(
...
pub fn get_vga_mem_addr() -> Result<(PhysicalAddress, usize, PteFlags), &'static str> {
...
fn convert_to_pte_flags(section: &impl ElfSection) -> PteFlags {
    use crate::sys::boot_info::ElfSectionFlags;
    PteFlags::new()
