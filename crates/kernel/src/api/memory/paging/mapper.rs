...
use crate::api::memory::mem::{BROADCAST_TLB_SHOOTDOWN_FUNC, VirtualAddress, PhysicalAddress, Page, Frame, FrameRange, AllocatedPages, AllocatedFrames}; 
use super::{
    get_current_p4,
    PageRange,
...
use spin::Once;
use crate::api::config::memory::PAGE_SIZE;
use super::tlb_flush_virt_addr;
use zerocopy::FromBytes;
use crate::sys::page_table_entry::UnmapResult;
use owned_borrowed_trait::{OwnedOrBorrowed, Owned, Borrowed};

#[cfg(target_arch = "x86_64")]
use crate::api::config::memory::ENTRIES_PER_PAGE_TABLE;
...
impl Mapper {
    ...
    pub fn map_allocated_pages<F: Into<PteFlagsArch>>(
        ...
        for page in pages.deref().clone() {
            let af = crate::sys::frame_alloc::allocate_frames(1).ok_or("map_allocated_pages(): couldn't allocate new frame, out of memory")?;
...
impl MappedPages {
    ...
    pub fn deep_copy<F: Into<PteFlagsArch>>(
        ...
        println!("MappedPages::deep_copy() has not been adequately tested yet.");
    ...
    pub fn remap<F: Into<PteFlagsArch>>(
        ...
        if new_flags == self.flags {
            println!("remap(): new_flags were the same as existing flags, doing nothing.");
            return Ok(());
        }
        ...
        for page in self.pages.clone() {            
            ...
            match unmapped_frames {
                        ...
                        match curr_frames.merge(newly_unmapped_frames) {
                            ...
                            Err(newly_unmapped_frames) => {
                                ...
                                if first_frame_range.is_none() {
                                    ...
                                    println!("MappedPages::unmap(): dropping additional non-contiguous frames {:?}", curr_frames);
                ...
                UnmapResult::NonExclusive(_frames) => {
                    // println!("Note: FYI: page {:X?} -> frames {:X?} was just unmapped but not mapped as EXCLUSIVE.", page, _frames);
                }
            }
        }
    ...
    pub fn as_type<T: FromBytes>(&self, byte_offset: usize) -> Result<&T, &'static str> {
        ...
        if false {
            println!("MappedPages::as_type(): requested type {} with size {} at byte_offset {}, MappedPages size {}!",
    ...
    pub fn as_type_mut<T: FromBytes>(&mut self, byte_offset: usize) -> Result<&mut T, &'static str> {
        let size = mem::size_of::<T>();
        if false {
            println!("MappedPages::as_type_mut(): requested type {} with size {} at byte_offset {}, MappedPages size {}!",
                core::any::type_name::<T>(),
                size, byte_offset, self.size_in_bytes()
            );
        }
    ...
    pub fn as_slice<T: FromBytes>(&self, byte_offset: usize, length: usize) -> Result<&[T], &'static str> {
        ...
        if false {
            println!("MappedPages::as_slice(): requested slice of type {} with length {} (total size {}) at byte_offset {}, MappedPages size {}!",
                core::any::type_name::<T>(),
    ...
    pub fn as_slice_mut<T: FromBytes>(&mut self, byte_offset: usize, length: usize) -> Result<&mut [T], &'static str> {
        ...
        if false {
            println!("MappedPages::as_slice_mut(): requested slice of type {} with length {} (total size {}) at byte_offset {}, MappedPages size {}!",
                core::any::type_name::<T>(), 
                length, size_in_bytes, byte_offset, self.size_in_bytes()
            );
        }
...
impl Drop for MappedPages {
    fn drop(&mut self) {
        ...
        //     println!("MappedPages::drop(): unmapped MappedPages {:?}, flags: {:?}", &*self.pages, self.flags);
        // }
