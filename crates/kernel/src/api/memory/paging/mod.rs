...
pub use crate::sys::page_table_entry::PageTableEntry;

use crate::{
    api::memory::mem::EarlyIdentityMappedPages,
    sys::{frame_alloc, page_alloc},
};
...
use crate::api::config::memory::{
    PAGE_SIZE, RECURSIVE_P4_INDEX, UPCOMING_PAGE_TABLE_RECURSIVE_P4_INDEX,
};
use crate::api::memory::mem::{
    allocate_pages, find_section_memory_bounds, get_p4, tlb_flush_all, tlb_flush_virt_addr,
    AllocatedFrames, AllocatedPages, Frame, FrameRange, InitialMemoryMappings, PageRange,
    PhysicalAddress, PteFlags, VirtualAddress,
};
use crate::sys::boot_info::BootInformation;
use crate::sys::no_drop::NoDrop;
use crate::sys::pte_flags::PteFlagsArch;

#[cfg(target_arch = "aarch64")]
use memory_aarch64::set_as_active_page_table_root;

#[cfg(target_arch = "x86_64")]
use super::memory_x86_64::get_vga_mem_addr;
...
impl PageTable {
    ...
    fn from_current() -> Result<PageTable, ()> {
        let current_p4 =
            crate::sys::frame_alloc::allocate_frames_at(get_current_p4().start_address(), 1)
                .map_err(|_| ())?;
    ...
    pub fn new_table(
        ...
        let mut temporary_page =
            TemporaryPage::create_and_map_table_frame(page, new_p4_frame, current_page_table)?;
    ...
    pub fn with<F, R>(&mut self, other_table: &mut PageTable, f: F) -> Result<R, &'static str>
    where
    ...
    pub fn switch(&mut self, new_table: &PageTable) {
        // println!("PageTable::switch() old table: {:?}, new table: {:?}", self, new_table);
...
pub fn init(
    ...
    let mut page_table = PageTable::from_current().map_err(|_| {
        "Failed to allocate frame for initial page table; is it merged with another section?"
    })?;
    println!("Bootstrapped initial {:?}", page_table);

    let (aggregated_section_memory_bounds, _sections_memory_bounds) =
        find_section_memory_bounds(boot_info, |virtual_address| {
            page_table.translate(virtual_address)
        })?;
    println!(
        "{:X?}\n{:X?}",
    ...
    let boot_info_start_vaddr = boot_info
        .start()
    ...
    let new_p4_frame = crate::sys::frame_alloc::allocate_frames(1)
        .ok_or("couldn't allocate frame for new page table")?;
    ...
    println!("Initial stack start {stack_start_virt:#X}, size: {stack_size:#X} bytes, {stack_page_range:X?}");
    ...
    println!("Boot info start: {boot_info_start_vaddr:#X}, size: {boot_info_size:#X}, {boot_info_page_range:#X?}");
    ...
    let (
        ...
        println!("{:X?}", aggregated_section_memory_bounds);
        ...
        let init_pages = page_alloc::allocate_pages_by_bytes_at(
            init_start_virt,
            init_end_virt.value() - init_start_virt.value(),
        )?;
        let init_frames = frame_alloc::allocate_frames_by_bytes_at(
            init_start_phys,
            init_end_phys.value() - init_start_phys.value(),
        )?;
        let init_pages_identity = page_alloc::allocate_pages_by_bytes_at(
        ...
        let mut init_mapped_pages =
            new_mapper.map_allocated_pages_to(init_pages, init_frames, init_flags)?;

        let text_pages = page_alloc::allocate_pages_by_bytes_at(
            text_start_virt,
            text_end_virt.value() - text_start_virt.value(),
        )?;
        let text_frames = frame_alloc::allocate_frames_by_bytes_at(
            text_start_phys,
            text_end_phys.value() - text_start_phys.value(),
        )?;
        let text_pages_identity = page_alloc::allocate_pages_by_bytes_at(
        ...
        init_mapped_pages
            .merge(new_mapper.map_allocated_pages_to(text_pages, text_frames, text_flags)?)
            .map_err(|(error, _)| error)?;
        let text_mapped_pages = NoDrop::new(init_mapped_pages);

        let rodata_pages = page_alloc::allocate_pages_by_bytes_at(
            rodata_start_virt,
            rodata_end_virt.value() - rodata_start_virt.value(),
        )?;
        let rodata_frames = frame_alloc::allocate_frames_by_bytes_at(
            rodata_start_phys,
            rodata_end_phys.value() - rodata_start_phys.value(),
        )?;
        let rodata_pages_identity = page_alloc::allocate_pages_by_bytes_at(
        ...
        let rodata_mapped_pages = NoDrop::new(new_mapper.map_allocated_pages_to(
            rodata_pages,
            rodata_frames,
            rodata_flags,
        )?);

        let data_pages = page_alloc::allocate_pages_by_bytes_at(
            data_start_virt,
            data_end_virt.value() - data_start_virt.value(),
        )?;
        let data_frames = frame_alloc::allocate_frames_by_bytes_at(
            data_start_phys,
            data_end_phys.value() - data_start_phys.value(),
        )?;
        let data_pages_identity = page_alloc::allocate_pages_by_bytes_at(
        ...
        let data_mapped_pages =
            NoDrop::new(new_mapper.map_allocated_pages_to(data_pages, data_frames, data_flags)?);
        ...
        let stack_guard_page = page_alloc::allocate_pages_at(stack_start_virt, 1)?;
        ...
        for page in stack_page_range.into_iter() {
            ...
            let frame = current_mapper
                .translate_page(page)
                .ok_or("couldn't translate stack page")?;
            let allocated_page = page_alloc::allocate_pages_at(page.start_address(), 1)?;
            let allocated_frame = frame_alloc::allocate_frames_at(frame.start_address(), 1)?;
            let mapped_pages =
                new_mapper.map_allocated_pages_to(allocated_page, allocated_frame, data_flags)?;
            if let Some(ref mut stack_mapped_pages) = stack_mapped_pages {
                stack_mapped_pages
                    .merge(mapped_pages)
                    .map_err(|_| "failed to merge stack mapped pages")?;
            } else {
        ...
        let additional_mapped_pages = {
            ...
            let vga_display_pages_identity = page_alloc::allocate_pages_by_bytes_at(
                VirtualAddress::new_canonical(vga_phys_addr.value()),
                vga_size_in_bytes,
            )?;
            let vga_display_frames =
                frame_alloc::allocate_frames_by_bytes_at(vga_phys_addr, vga_size_in_bytes)?;
        ...
        for page in boot_info_page_range.into_iter() {
            ...
            let allocated_page = page_alloc::allocate_pages_at(page.start_address(), 1)?;
            let allocated_frame = frame_alloc::allocate_frames_at(frame.start_address(), 1)?;
            let mapped_pages = new_mapper.map_allocated_pages_to(
                allocated_page,
                allocated_frame,
                PteFlags::new(),
            )?;
            if let Some(ref mut boot_info_mp) = boot_info_mapped_pages {
                boot_info_mp
                    .merge(mapped_pages)
                    .map_err(|_| "failed to merge boot info pages")?;
            } else {
        ...
        println!("{identity_mapped_pages:?}");
        println!("{additional_mapped_pages:?}");
        Ok((
            text_mapped_pages,
            rodata_mapped_pages,
            data_mapped_pages,
