...
use crate::sys::{pte_flags::PteFlagsArch, page_alloc};
use crate::api::config::memory::{TEMPORARY_PAGE_VIRT_ADDR, PAGE_SIZE};
...
impl TemporaryPage {
    ...
    pub fn create_and_map_table_frame(
        ...
        while page.is_none() && vaddr.value() != 0 {
            page = page_alloc::allocate_pages_at(vaddr, 1).ok();
            vaddr -= PAGE_SIZE;
        }
