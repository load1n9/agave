use super::{ElfSectionFlags, FramebufferFormat};
use core::iter::{Iterator, Peekable};
use crate::api::config::memory::{KERNEL_STACK_SIZE_IN_PAGES, PAGE_SIZE};
use crate::api::memory::structs::{PhysicalAddress, VirtualAddress};
use agave_uefi_bootloader_api::PixelFormat;

pub const STACK_SIZE: usize = (KERNEL_STACK_SIZE_IN_PAGES + 2) * PAGE_SIZE;

const MODULES_MEMORY_KIND: agave_uefi_bootloader_api::MemoryRegionKind =
    agave_uefi_bootloader_api::MemoryRegionKind::UnknownUefi(0x80000000);

impl super::MemoryRegion for agave_uefi_bootloader_api::MemoryRegion {
    ...
    fn is_usable(&self) -> bool {
        matches!(self.kind, agave_uefi_bootloader_api::MemoryRegionKind::Usable)
    }
}

pub struct MemoryRegions<'a> {
    inner: Peekable<core::slice::Iter<'a, agave_uefi_bootloader_api::MemoryRegion>>,
}

impl<'a> Iterator for MemoryRegions<'a> {
    type Item = agave_uefi_bootloader_api::MemoryRegion;
...
impl<'a> super::ElfSection for &'a agave_uefi_bootloader_api::ElfSection {
    fn name(&self) -> &str {
        agave_uefi_bootloader_api::ElfSection::name(self)
    }
...
#[derive(Debug)]
pub struct Module {
    inner: agave_uefi_bootloader_api::Module,
    regions: &'static agave_uefi_bootloader_api::MemoryRegions,
}

impl super::Module for Module {
    fn name(&self) -> Result<&str, &'static str> {
        Ok(agave_uefi_bootloader_api::Module::name(&self.inner))
    }
...
pub struct Modules {
    inner: &'static agave_uefi_bootloader_api::Modules,
    regions: &'static agave_uefi_bootloader_api::MemoryRegions,
    index: usize,
}
...
impl super::BootInformation for &'static agave_uefi_bootloader_api::BootInformation {
    type MemoryRegion<'a> = agave_uefi_bootloader_api::MemoryRegion;
    type MemoryRegions<'a> = MemoryRegions<'a>;

    type ElfSection<'a> = &'a agave_uefi_bootloader_api::ElfSection;
    type ElfSections<'a> = core::slice::Iter<'a, agave_uefi_bootloader_api::ElfSection>;

    type Module<'a> = Module;
    type Modules<'a> = Modules;

    type AdditionalReservedMemoryRegions = core::iter::Empty<super::ReservedMemoryRegion>;
    ...
    fn rsdp(&self) -> Option<PhysicalAddress> {
        self.rsdp_address.map(PhysicalAddress::new_canonical)
    }
    ...
    fn framebuffer_info(&self) -> Option<super::FramebufferInfo> {
