// ported from https://github.com/theseus-os/Theseus/blob/theseus_main/kernel/frame_allocator/src/lib.rs
extern crate alloc;
extern crate spin;
...
use crate::api::{memory::structs::{PhysicalAddress, Frame, FrameRange}, config::memory::PAGE_SIZE};
use modular_bitfield::private::static_assertions::assert_not_impl_any;
use spin::Mutex;
                        ...
                        let next_contiguous_chunk: Option<Chunk> = {
                            ...
                            if let Some(next_chunk) = next_cursor.get().map(|w| w.deref()) {
                                if *chunk.end() + 1 == *next_chunk.start() {
                                    ...
                                    println!("Frame allocator: next {:?} was not contiguously above initial too-small {:?}", next_chunk, chunk);
                                    None
                                }
                            } else {
                               println!("Frame allocator: couldn't get next chunk above initial too-small {:?}", chunk);
                                println!("Requesting new chunk starting at {:?}, num_frames: {}", *chunk.end() + 1, requested_end_frame.number() - chunk.end().number());
    ...
    match list.0 {
        ...
        Inner::RBTree(ref mut tree) => {
            ...
            while let Some(chunk) = cursor.get().map(|w| w.deref()) {
                ...
                println!("Frame allocator: inefficient scenario: had to search multiple chunks \
...
pub fn allocate_frames_deferred(
    ...
    if num_frames == 0 {
        println!("frame_allocator: requested an allocation of 0 frames... stupid!");
                ...
                AllocationError::AddressNotFound(..) => {
                    ...
                    match find_specific_chunk(&mut FREE_GENERAL_FRAMES_LIST.lock(), start_frame, num_frames) {
                        ...
                        Err(AllocationError::ContiguousChunkNotFound(..)) => {
                            println!("Only part of the requested allocation was found in the general frames list.");
...
pub fn allocate_frames_by_bytes_deferred(
    ...
    let num_frames = (actual_num_bytes + FRAME_SIZE - 1) / FRAME_SIZE;
