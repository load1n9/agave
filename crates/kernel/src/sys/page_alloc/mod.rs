// ported from https://github.com/theseus-os/Theseus/blob/theseus_main/kernel/page_allocator/src/lib.rs
...
use crate::api::memory::structs::{Page, PageRange, VirtualAddress};
use core::{
    borrow::Borrow,
    cmp::Ordering,
    fmt,
...
static DESIGNATED_PAGES_HIGH_START: Page = Page::containing_address(VirtualAddress::new_canonical(
    UPCOMING_PAGE_TABLE_RECURSIVE_MEMORY_START,
));
    ...
    match list.0 {
        ...
        Inner::RBTree(ref mut tree) => {
            ...
            while let Some(chunk) = cursor.get().map(|w| w.deref()) {
                ...
                println!("Page allocator: unlikely scenario: had to search multiple chunks while trying to allocate {} pages at any address.", num_pages);
                cursor.move_prev();
            }
        }
    }
...
pub fn allocate_pages_deferred(
    ...
    if num_pages == 0 {
        println!("PageAllocator: requested an allocation of 0 pages... stupid!");
