...
impl Editor {
    ...
    fn print_screen(&mut self) {
        ...
        let color = Style::color("White").with_background("Black");
        let reset = Style::reset();
        println!("{}\x1b[1;1H{}{}", color, rows.join("\n"), reset);
    }
    ...
    fn render_line(&self, y: usize) -> String {
        ...
        let mut row: Vec<char> = format!("{:cols$}", line, cols = self.offset.x)
            .chars()
            .collect();
...
fn truncated_line_indicator() -> String {
    let color = Style::color("Black").with_background("Cyan");
    let reset = Style::reset();
    format!("{}>{}", color, reset)
}
