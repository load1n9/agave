...
fn help_summary() -> Result<(), ExitCode> {
    ...
    println!(" Dean Srebnik 2023");
    println!(" A huge amount of code used from MOROS by Vincent Ollivier <v@vinc.cc>");
    Ok(())
}
...
fn help_date() -> Result<(), ExitCode> {
    ...
    let specifiers = [
        ...
        (
            "%c",
            "Date and time representation, equivalent to %a %b %-d %-H:%M:%S %-Y",
        ...
        (
            "%U",
            "Week number with the first Sunday as the start of week one (00-53)",
            "33",
        ),
        ...
        (
            "%W",
            "Week number with the first Monday as the start of week one (00-53)",
            "34",
        ),
